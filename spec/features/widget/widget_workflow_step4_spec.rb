...
feature 'WIDGET WORKFLOW STEP 4' do
  ...
  scenario 'clicking on confirm button for selected value saves it', js: true do
    ...
    expect(webpage.item_name_id.to_s).to include('follower_widget__test_item_name')

    visit workspace_webpage_path(webpage)

    expect(page).to have_selector('td', text: 'follower_widget__test_item_name')
  end
